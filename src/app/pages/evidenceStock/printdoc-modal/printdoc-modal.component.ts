...
export class PrintDocModalComponent implements OnInit {
    printDoc = [
        {
            IsChecked: false,
            DocName: 'จำนวนคงเหลือในคลัง',
            DocType: 1,
            DocTypeName: 'แบบฟอร์ม',
        },
        {
            IsChecked: false,
            DocName: 'ประวัติของกลาง',
            DocType: 1,
            DocTypeName: 'แบบฟอร์ม',
        }
    ]
    ...
    constructor(
        private EviStockService: EvidenceStockService,
        private preloader: PreloaderService
    ) { }

    ngOnInit() {
        //this.printDoc = this.printDoc.filter(x => x.EvidenceType == this.Evitype);
    }
