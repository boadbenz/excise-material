...
>>>>>>> master
  ...
  private getByCon(code: string) {
    this.arrestService.getByCon(code).then(async res => {
      ...
      await res.ArrestLocale.map(
        item =>
          (item.Region = `${item.SubDistrict} ${item.District} ${
            item.Province
          }`)
      );
      ...
      await res.ArrestLawbreaker.map(item => {
        ...
        item.LawbreakerFullName += ` ${item.LawbreakerMiddleName} ${
  ...
  private onReviced() {
    ...
    this.arrestService.updByCon(this.arrestForm.value).then(async res => {
      if (res.IsSuccess === true) {
        ...
        await staff.filter(item => item.IsNewItem === true).map(item => {
          this.arrestService
            .staffinsAll(item)
            .then(s => (isSuccess = s.IsSuccess));
          if (isSuccess === false) {
            return false;
          }
        });
        ...
        await lawbreaker.filter(item => item.IsNewItem === true).map(item => {
          this.arrestService
            .lawbreakerinsAll(item)
            .then(s => (isSuccess = s.IsSuccess));
          if (isSuccess === false) {
            return false;
          }
        ...
        await product.filter(item => item.IsNewItem === true).map(item => {
          this.arrestService
            .productinsAll(item)
            .then(s => (isSuccess = s.IsSuccess));
          if (isSuccess === false) {
...
<<<<<<< HEAD
        ...
        await indicment.filter(item => item.IsNewItem === true).map(item => {
          this.arrestService
            .indicmentinsAll(item)
            .then(s => (isSuccess = s.IsSuccess));
          if (isSuccess === false) {
            return false;
          }
